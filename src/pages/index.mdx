---
layout: ../layouts/main.astro
title: SnowCamp
description: home page
---
import Utils from '../utils';
import current from '../data/current.yaml';
import Polaroid from '../components/Polaroid.astro';
import Spotlight from '../components/Spotlight.astro';
import Columns3 from '../components/Columns3.astro';
import ShortDescription from '../components/ShortDescription.astro';
import TimelineCFP from '../components/TimelineCFP.astro';
import Button from '../components/Button.astro';
import Sponsor from '../components/Sponsor.astro';
import sponsors from '../data/sponsors.yaml';

export const sponsorsTable = sponsors.sponsors.reduce((a, b) => {
  a[b.id] = b;
  return a;
}, {});

export const startFormat = Intl.DateTimeFormat('fr', {
  month: 'long',
  day: 'numeric',
});
export const endFormat = Intl.DateTimeFormat('fr', {
  year: 'numeric',
  month: 'long',
  day: 'numeric',
});
export const opencfp = endFormat.format(Utils.parseDate(current.timeline.cfp.open));
export const startDate = Utils.parseDate(current.start_date);
export const start = startFormat.format(startDate);
export const end = endFormat.format(Utils.parseDate(current.end_date));
export const year = startDate.getFullYear();

export const etoiles = current.sponsorship.etoile.map((id) => sponsorsTable[id]);
export const flocons = current.sponsorship.flocon.map((id) => sponsorsTable[id]);
export const partners = current.sponsorship.partners.map((id) => sponsorsTable[id]);
export const tlCFP = [
  {
    "title": "Ouverture",
    "date": current.timeline.cfp.open,
  },
  {
    "title": "Fermeture",
    "date": current.timeline.cfp.close,
  },
  {
    "title": "Notification",
    "date": current.timeline.cfp.notification,
  },
  {
    "title": "Conférence",
    "begin": current.start_date,
    "end": current.end_date,
  }
];

<Spotlight/>

# Snowcamp

> 2 jours de conférences avec des présentations de 45 minutes.

<Columns3>
  <ShortDescription title="Universités" description="Une journée de formation en profondeur (2 formations de 3 heures au choix)." img={Utils.url("/img/icons.avif")} offset="-500px"/>
  <ShortDescription title="Conférences" description="2 jours de conférences avec des présentations de 45 minutes." img={Utils.url("/img/icons.avif")} offset="-200px"/>
  <ShortDescription title="Unconference" description="Une journée d'échange sur les pistes mêlant orateurs et participants." img={Utils.url("/img/icons.avif")} offset="-400px"/>
</Columns3>

# Innovation & Recherche

> Une conférence à haut niveau technique réunissant des ingénieurs et des académiques

<Columns3>
  <ShortDescription title="Innovation" description="Découvrez les outils, frameworks et technologies d'aujourd'hui et de demain." img={Utils.url("/img/icons.avif")} offset="-300px"/>
  <ShortDescription title="Échange" description="Rencontrez et échangez avec d'autres devs, ops, archis et orateurs." img={Utils.url("/img/icons.avif")} offset="-100px"/>
  <ShortDescription title="Recherche" description="Découvrez les travaux des chercheurs sur les tendances et sujets du moment." img={Utils.url("/img/icons.avif")} offset="0"/>
</Columns3>

# Call For Proposals

<TimelineCFP tl={tlCFP}/>

<div style="display: flex; flex-flow: row wrap; align-items: center; gap: 2rem; justify-content: space-evenly">
Vous pouvez trouver tous les [détails ici](/cfp).
</div>

<a id="sponsors"></a>

# Sponsors { year }

> Ils nous ont déjà accordé leur confiance pour préparer ensemble l'édition { year }.

## Étoile

<div style="display: flex; flex-flow: row wrap; align-items: center; gap: 2rem; justify-content: space-evenly">
{ etoiles.map((etoile) => <Sponsor title={etoile.name} img={etoile.logo} href={etoile.link.fr} />)
}
</div>

## Flocon

<div style="display: flex; flex-flow: row wrap; align-items: center; gap: 2rem; justify-content: space-evenly">
{ flocons.map((flocon) => <Sponsor title={flocon.name} img={flocon.logo} href={flocon.link.fr} />)
}
</div>

> Un grand merci à nos sponsors !

<Button className="sponsorOpened" title="Devenez sponsor !" href={current.sponsorship.subscribe_link} type="ext"/>

## Partenaires

<div style="display: flex; flex-flow: row wrap; align-items: center; gap: 2rem; justify-content: space-evenly">
{ partners.map((partner) => <Sponsor title={partner.name} img={partner.logo} href={partner.link.fr} />)
}
</div>

<a id="venue"></a>

# Venir

> SnowCamp se déroulera à *Alpexpo, dans l'espace 1968*<br/>à la fois pour les Universités et les Conférences.

<p>Les Universités et les Conférences auront toutes lieux à *["Alpexpo, espace 1968"](https://alpexpo.com/)*.</p>

Av. d'Innsbruck<br/>
38100 Grenoble

<div>
  <Polaroid img="/img/alpexpo/alpexpo-espace-1968-300.avif" title="Alpexpo - Espace 1968 (Crédit photo Alpexpo)" side="left" />
Vous pouvez y accéder :

- Voiture par Rocade Sud, Sortie 6 Alpexpo, parking gratuit
- Train ou bus : Tram A (arrêt Pôle Sud - Alpexpo), Bus C3 (Arret Alpexpo), C6, C8, 12, 65, 67 (Arret Grand Place)
- Avion : il y a des navettes d'aéroports : depuis Lyon Saint-Exupéry (17 rotations 7j/7) et depuis Genève Cointrin (3 rotations, 7j/7)
- [Vélo](https://pass.mobilites-m.fr/accueil/velo)
</div>

