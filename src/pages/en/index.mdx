---
layout: ../../layouts/main.astro
title: SnowCamp
description: home page
---
import Utils from '../../utils.js';
import current from '../../data/current.yaml';
import Polaroid from "../../components/Polaroid.astro";
import Spotlight from '../../components/Spotlight.astro';
import Columns3 from '../../components/Columns3.astro';
import ShortDescription from '../../components/ShortDescription.astro';
import TimelineCFP from '../../components/TimelineCFP.astro';
import Button from '../../components/Button.astro';
import Sponsor from '../../components/Sponsor.astro';
import sponsors from '../../data/sponsors.yaml';

export const sponsorsTable = sponsors.sponsors.reduce((a, b) => {
  a[b.id] = b;
  return a;
}, {});

export const startFormat = Intl.DateTimeFormat('en', {
  month: 'long',
  day: 'numeric',
});
export const endFormat = Intl.DateTimeFormat('en', {
  year: 'numeric',
  month: 'long',
  day: 'numeric',
});
export const opencfp = endFormat.format(Utils.parseDate(current.timeline.cfp.open));
export const startDate = Utils.parseDate(current.start_date);
export const start = startFormat.format(startDate);
export const end = endFormat.format(Utils.parseDate(current.end_date));
export const year = startDate.getFullYear();

export const etoiles = current.sponsorship.etoile.map((id) => sponsorsTable[id]);
export const flocons = current.sponsorship.flocon.map((id) => sponsorsTable[id]);
export const partners = current.sponsorship.partners.map((id) => sponsorsTable[id]);
export const tlCFP = [
  {
    "title": "Open",
    "date": current.timeline.cfp.open,
  },
  {
    "title": "Close",
    "date": current.timeline.cfp.close,
  },
  {
    "title": "Notification",
    "date": current.timeline.cfp.notification,
  },
  {
    "title": "Conference",
    "begin": current.start_date,
    "end": current.end_date,
  }
];

<Spotlight/>

# Snowcamp

> 2 days of conferences with 45mn talks.

<Columns3>
  <ShortDescription title="Universities" description="A day of training (choice of 2 trainings of 3 hours long)." img={Utils.url("/img/icons.avif")} offset="-500px"/>
  <ShortDescription title="Conferences" description="2 days of conferences with 45mn talks." img={Utils.url("/img/icons.avif")} offset="-200px"/>
  <ShortDescription title="Unconference" description="A day of exchange on the skiing slopes gathering speakers and attendees." img={Utils.url("/img/icons.avif")} offset="-400px"/>
</Columns3>

# Innovation and research

> A high-level technical conference gathering engineers and researchers

<Columns3>
  <ShortDescription title="Innovation" description="Learn about today's and tomorrow's tools, frameworks and technologies." img={Utils.url("/img/icons.avif")} offset="-300px"/>
  <ShortDescription title="Exchange" description="Meet and exchange with other devs, ops, architects and speakers." img={Utils.url("/img/icons.avif")} offset="-100px"/>
  <ShortDescription title="Research" description="Discover trending works of researchers." img="/img/icon-academic.png" img={Utils.url("/img/icons.avif")} offset="0"/>
</Columns3>

# Call For Proposals

<TimelineCFP tl={tlCFP}/>

<div style="display: flex; flex-flow: row wrap; align-items: center; gap: 2rem; justify-content: space-evenly">
You can find more [details here](/en/cfp).
</div>

<a id="sponsors"></a>

# Sponsors { year }

> They have already placed their trust to prepare the edition { year } together.

## Étoile

<div style="display: flex; flex-flow: row wrap; align-items: center; gap: 2rem; justify-content: space-evenly">
{ etoiles.map((etoile) => <Sponsor title={etoile.name} img={etoile.logo} href={etoile.link.fr} />)
}
</div>

## Flocon

<div style="display: flex; flex-flow: row wrap; align-items: center; gap: 2rem; justify-content: space-evenly">
{ flocons.map((flocon) => <Sponsor title={flocon.name} img={flocon.logo} href={flocon.link.fr} />)
}
</div>

> Many thanks to our sponsors!

<Button title="Become sponsor !" href={current.sponsorship.subscribe_link} type="ext"/>

## Parteners

<div style="display: flex; flex-flow: row wrap; align-items: center; gap: 2rem; justify-content: space-evenly">
{ partners.map((partner) => <Sponsor title={partner.name} img={partner.logo} href={partner.link.en} />)
}
</div>

<a id="venue"></a>

# Venue

<Polaroid img="/img/alpexpo/alpexpo.jpg" title="Alpexpo Grenoble" side="left" />

> The schedule can be found here:

Universities and conferences will take place at *["Alpexpo, espace 1968"](https://alpexpo.com/)*.

5 - 7 , place Robert Schuman<br/>
38025 Grenoble

<div>
  <Polaroid img="/img/alpexpo/alpexpo-espace-1968-300.avif" title="Alpexpo - Espace 1968 (Crédit photo Alpexpo)" side="left" />
You can access:

- Car road Rocade Sud, Exit 6 Alpexpo, free parking
- Train or bus: Tram A (stop Pôle Sud - Alpexpo), Bus C3 (stop Alpexpo), Bus C6, C8, 12, 65, 67 (stop Grand Place)
- Avion: il y a des navettes d'aéroports : depuis Lyon Saint-Exupéry (17 rotations 7j/7) et depuis Genève Cointrin (3 rotations, 7j/7)
- by [bike](https://pass.mobilites-m.fr/accueil/velo)
</div>
